...
- [X] T021 **Resolved, not just verified.** The real Coolify deployment (this repo's actual production target) failed three times in a row with `@pydantic/monty`'s native backend — root-caused via SSH access to the production host: a GLIBC 2.38 requirement unmet by Coolify's Debian Bookworm build base (2.36). Fixed by switching to `@pydantic/monty/wasm` (research.md §2). Verified conclusively in the real environment, not inferred: a full `npm ci` + `npm run build` + `npm run start` (with T017's boot-time smoke test) run end-to-end inside a `node:24-bookworm` container on the actual Coolify host, all succeeding. Vercel specifically was not re-verified after this switch (the user clarified Coolify, not Vercel, is the platform that matters here) but WASM has no platform-specific binary at all, so there's no reason to expect a Vercel-specific failure mode.
