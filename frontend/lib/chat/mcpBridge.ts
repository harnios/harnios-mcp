...
import { callTool, type ToolCallResult } from "@/lib/mcp-tools/inProcessClient";
...
export async function discoverFromClient(
  client: Client,
  executeTool: (name: string, args: Record<string, unknown>) => Promise<ToolCallResult> = (name, args) => callTool(client, name, args),
  ...
  return Object.fromEntries(tools.map((definition) => {
    return [definition.name, tool({
      ...
      execute: async (input) => {
        const result = await executeTool(definition.name, input as Record<string, unknown>);
