import type { Dictionary } from "./types";

/** Canonical English dictionary — the key source of truth every other language mirrors (research.md §6). */
export const en: Dictionary = {
  chat: { open: "Open chat", close: "Close chat", title: "Harnios assistant", placeholder: "Ask Harnios…", send: "Send", thinking: "Thinking…", error: "Something went wrong.", maximize: "Maximize chat", restore: "Restore chat", toolRunning: "Running tool", toolComplete: "Tool completed", toolFailed: "Tool failed", toolPending: "Waiting", toolInput: "Request", toolResult: "Result", toolDetails: "Show tool details", stop: "Stop", reset: "Reset chat", modeLabel: "Chat mode", modeHarnios: "Harnios", modeGeneral: "General", modeLocked: "Wait for the current operation to finish before changing mode" },
  init: {
    setupTitle: "Set up your Company OS",
    setupDescription:
      "This creates the starting structure — os/, data/, and AGENTS.md. No details are asked here; once connected, your AI assistant builds the rest through its own MCP connection and interviews you for the details.",
    languagePrompt:
      "Pick the language for your Company OS below — this choice is permanent for this Company OS once confirmed.",
    submit: "Initialize Company OS",
    unexpectedStateTitle: "Unexpected storage state",
    unexpectedStateBody:
      "This storage is in an unexpected, partially-initialized state — only one of os/ or data/ exists. No setup action is available here; resolve this manually before continuing.",
    mcpConnect: {
      readyTitle: "Your Company OS is ready",
      connectTitle: "Connect Claude or ChatGPT",
      justCreatedText:
        "The starting structure (os/, data/, AGENTS.md) has been created. Connect an AI assistant below, then ask it to initialize the Company OS — it reaches the setup process through its own MCP connection and will interview you and set up the rest.",
      step1: "In Claude or ChatGPT's \"add connector\"/\"add MCP server\" flow, enter:",
      step2: "The assistant discovers the OAuth flow automatically — sign in with your owner credential when prompted, then approve the connection.",
      step3: "Once connected, ask the assistant to initialize the Company OS to get started.",
      reviewText: "Review or revoke connected assistants at /settings/connected-apps. For scripts/CLI tools instead of a hosted assistant, use a personal access token instead of OAuth.",
      goToEditor: "Go to /files",
    },
    submitUnauthorized: "Sign in required",
    submitInvalidLanguage: "Select one of the six supported languages.",
    envSetup: {
      title: "Set up your environment",
      description:
        "This app needs a storage connection and an owner sign-in credential to work. Fill in the fields below to generate a ready-to-use configuration snippet — nothing you type here is ever sent anywhere; it only stays in this browser tab.",
      connectionErrorHeading: "Storage connection failed",
      storageHeading: "Storage connection",
      endpoint: "Endpoint",
      region: "Region",
      accessKeyId: "Access key ID",
      secretAccessKey: "Secret access key",
      bucket: "Bucket",
      pathStyleLabel:
        "Use path-style addressing (required by most self-hosted S3-compatible servers, including MinIO)",
      ownerHeading: "Owner sign-in",
      ownerDescription: "Used to sign in and approve AI assistants, manage tokens, and use the file editor.",
      username: "Username",
      password: "Password",
      systemNameHeading: "System name (optional)",
      systemNameLabel: "What's your system called?",
      messagingHeading: "Email & Telegram messaging (optional)",
      messagingDescription:
        "Lets the assistant send emails and Telegram messages on your behalf. Skip this section if you don't need it yet — you can always add it later.",
      smtpSubheading: "Email (SMTP)",
      smtpHost: "SMTP host",
      smtpPort: "SMTP port",
      smtpSecureLabel: "Use implicit TLS (leave unchecked for STARTTLS, the default for port 587)",
      smtpUser: "SMTP username",
      smtpPassword: "SMTP password",
      smtpFrom: "Sender address",
      telegramSubheading: "Telegram",
      telegramBotToken: "Bot token",
      telegramChatId: "Default chat ID (optional)",
      rateLimitSubheading: "Shared send limit",
      rateLimitMax: "Max messages",
      rateLimitWindowMinutes: "Per minutes",
      configHeading: "Configuration",
      copy: "Copy",
      copied: "Copied!",
      applyHeading: "Apply it",
      applyLocallyLabel: "Locally",
      applyLocallyText: ": paste it into frontend/.env.local, then restart the app (npm run dev).",
      applyVercelLabel: "On Vercel",
      applyVercelText:
        ": open your project, go to Settings → Environment Variables, and paste the lines above (Vercel accepts a full .env-formatted paste at once — or add each line as its own variable), then redeploy.",
      reloadNote: "Once applied, reload this page.",
    },
  },
  common: {
    homeLink: "Back to dashboard",
  },
  nav: {
    dashboard: "Dashboard",
    files: "Files",
    tools: "Tools",
    schedules: "Schedules",
    settings: "Settings",
    docs: "Docs",
    shares: "Shares",
    signOut: "Sign out",
    menuLabel: "Primary",
    home: "Home",
  },
  docs: {
    pageTitle: "Documentation",
    pageDescription: "How to use Harnios — pick a topic below.",
    topicsHeading: "Topics",
    topics: {
      overview: "Overview",
      dashboard: "Dashboard",
      files: "Files",
      tools: "Tools",
      schedules: "Schedules",
      settings: "Settings",
      shares: "Sharing",
    },
    notFoundTitle: "Topic not found",
    notFoundBody: "That documentation topic doesn't exist. Choose one of the topics below.",
    backToOverview: "Back to overview",
  },
  dashboard: {
    title: "Dashboard",
    description: "Everything this MCP server offers, in one place.",
    links: {
      files: "Files",
      tools: "Tools",
      externalMcpServers: "External MCP servers",
      settingsConnectedApps: "Settings — Connected apps",
      settingsPersonalAccessTokens: "Settings — Personal access tokens",
      settingsTestMessaging: "Settings — Test messaging tools",
      scheduledTasks: "Scheduled tasks",
    },
  },
  editor: {
    tree: {
      loading: "Loading…",
      empty: "(empty)",
      menuNewFile: "New file",
      menuNewBpmn: "New BPMN diagram",
      menuNewFolder: "New folder",
      menuUploadFiles: "Upload files",
      menuUploadFolder: "Upload folder",
      menuDownloadZip: "Download as zip",
      menuDeleteFolder: "Delete folder",
      menuDownload: "Download",
      menuDelete: "Delete",
      moreActions: "More actions",
      promptNewFile: "New file name:",
      promptNewBpmn: "New BPMN diagram name:",
      bpmnAlreadyExists: (name) => `"${name}" already exists. Choose another name.`,
      promptNewFolder: "New folder name:",
      invalidName: (name) => `"${name}" can't contain "/" — enter a plain name.`,
      nothingToUpload: "Nothing to upload.",
      nothingToUploadFiltered: (skipped) =>
        `Nothing to upload — none of the ${skipped} selected file(s) are a supported type.`,
      overwriteFilesConfirm: (names) => `This will overwrite existing file(s): ${names}. Continue?`,
      uploadFailed: (message) => `Upload failed: ${message}`,
      uploadFailedLabel: "Upload failed",
      uploadSummary: (uploaded, skipped) => `Uploaded ${uploaded} file(s), skipped ${skipped}.`,
      uploadSummaryFailedSuffix: (failedList) => ` Failed: ${failedList}`,
      downloadFailed: (message) => `Download failed: ${message}`,
      downloadNothing: "Nothing to download.",
      deleteFileConfirm: (name) => `Delete "${name}"? This can't be undone.`,
      deleteFailed: (message) => `Delete failed: ${message}`,
      deleteFailedLabel: "Delete failed",
      overwriteFileConfirm: (name) => `This will overwrite the existing file "${name}". Continue?`,
      createFailed: (message) => `Create failed: ${message}`,
      createFailedLabel: "Create failed",
      deleteFolderConfirm: (name) => `Delete folder "${name}" and everything inside it? This can't be undone.`,
      dirLoadFailed: "Failed to load directory",
    },
    file: {
      selectPrompt: "Select a file to view its content.",
      loading: (path) => `Loading "${path}"…`,
      preview: "Preview",
      table: "Table",
      edit: "Edit",
      raw: "Raw",
      diagram: "Diagram",
      xml: "XML",
      bpmnLoading: "Rendering BPMN diagram…",
      bpmnImportError: "This BPMN file could not be rendered. Open XML mode to inspect or correct it.",
      modelerTitle: "Edit BPMN diagram",
      modifyDiagram: "Modify diagram",
      apply: "Apply",
      cancel: "Cancel",
      close: "Close",
      discardModelerChanges: "Discard unapplied diagram changes?",
      modelerExportError: "The diagram could not be exported. Your changes are still open in the editor.",
      unsavedChanges: "● unsaved changes",
      save: "Save",
      saving: "Saving…",
      saved: "Saved",
      saveFailed: (message) => `Save failed: ${message}`,
      saveFailedLabel: "Save failed",
      discardConfirm: "You have unsaved changes. Discard them and open a different file?",
      loadFailed: "Failed to load file",
      openedPathIsFolder: (path) => `"${path}" is a folder, not a file.`,
      openOrDownload: "Open / Download",
      share: "Share temporarily",
      shareTitle: "Share file temporarily",
      shareDescription: "Create a read-only browser link that expires automatically.",
      shareCreate: "Create share link",
      shareCancel: "Cancel",
      shareExpiration: "Expiration",
      sharePassword: "Password (optional)",
      sharePasswordHint: "Anyone with the link can view it unless you add a password.",
      shareCopy: "Copy link",
      shareNative: "Share",
      shareCopied: "Link copied",
      shareLinkCreated: "Share link created. Copy it now; it will not be shown again.",
      shareFailed: (message) => `Share failed: ${message}`,
      shareManager: "Shared files",
      shareManagerDescription: "Manage temporary, read-only browser links.",
      shareEmpty: "No temporary shares yet.",
      sharePath: "File",
      shareExpires: "Expires",
      shareStatus: "Status",
      shareProtected: "Password protected",
      shareRevoke: "Revoke",
      shareRevoked: "Revoked",
      shareExpired: "Expired",
      shareActive: "Active",
      shareConfirmRevoke: "Revoke this share link?",
      externalChangeMessage: "This file changed externally.",
      externalChangeReload: "Reload external version",
      externalChangeKeepMine: "Keep mine",
    },
    csv: {
      empty: "This file is empty.",
      summary: (rows, columns) => `${rows.toLocaleString()} rows × ${columns.toLocaleString()} columns.`,
      truncated: (shown, total) => `Showing ${shown.toLocaleString()} of ${total.toLocaleString()} rows.`,
      noRows: "No data rows.",
    },
    header: {
      toggleSidebar: "Toggle file browser",
    },
  },
  settings: {
    connectedApps: {
      title: "Connected apps",
      empty: "No AI assistants are connected yet.",
      client: "Client",
      status: "Status",
      authorized: "Authorized",
      lastUsed: "Last used",
      never: "never",
      revoke: "Revoke",
      signOut: "Sign out",
    },
    pat: {
      title: "Personal access tokens",
      description:
        "Use a personal access token as a bearer credential for MCP clients that can't complete an OAuth sign-in.",
      empty: "No personal access tokens yet.",
      name: "Name",
      status: "Status",
      created: "Created",
      lastUsed: "Last used",
      never: "never",
      revoked: "revoked",
      active: "active",
      revoke: "Revoke",
      createTitle: "Create a new token",
      namePlaceholder: "Name (e.g. laptop, ci script)",
      generate: "Generate token",
      createdTitle: "Personal access token created",
      createdBody: "Copy this token now — it will not be shown again:",
      backLink: "Back to personal access tokens",
    },
    messagingTest: {
      title: "Test messaging tools",
      description:
        "Send a real test email or Telegram message through the server's configured accounts, and see exactly what the underlying service says on failure.",
      signOut: "Sign out",
      email: {
        sectionTitle: "Test email",
        toLabel: "Recipient",
        toPlaceholder: "someone@example.com",
        subjectLabel: "Subject",
        subjectPlaceholder: "Test",
        bodyLabel: "Body",
        bodyPlaceholder: "Hello from the test page",
        submit: "Send test email",
        sending: "Sending…",
        invalidRecipient: "Enter a valid email address.",
        emptyFields: "Subject and body must not be empty.",
        htmlToggle: "Send as HTML",
      },
      telegram: {
        sectionTitle: "Test Telegram message",
        chatIdLabel: "Chat ID",
        chatIdPlaceholder: "123456789 or @channelname",
        chatIdHint: "Leave blank to use the server's configured default chat.",
        textLabel: "Message text",
        textPlaceholder: "Test message",
        charCount: (count, max) => `${count} / ${max}`,
        submit: "Send test message",
        sending: "Sending…",
        emptyText: "Message text must not be empty.",
      },
      success: (destination) => `Sent successfully to ${destination}.`,
      failure: (errorCode, errorMessage) => `Failed (${errorCode}): ${errorMessage}`,
      recent: {
        heading: "Recent attempts",
        empty: "No test messages sent yet this session.",
        channel: "Channel",
        destination: "Destination",
        time: "Time",
        outcome: "Outcome",
        outcomeSuccess: "Success",
        outcomeFailure: "Failure",
      },
    },
  },
  tools: {
    title: "MCP tools",
    description: "Every tool this MCP server can offer, whether it's currently active or disabled, and a way to change that.",
    name: "Tool",
    group: "Group",
    status: "Status",
    active: "active",
    disabled: "disabled",
    signOut: "Sign out",
    disableAction: "Disable",
    enableAction: "Enable",
    confirmTitle: "Confirm change",
    confirmPendingChange: (name, to) => `You're about to set "${name}" to ${to}.`,
    confirmButton: "Confirm",
    cancelButton: "Cancel",
    warningNotice:
      "AI assistant sessions that were already connected before this change may keep using the previous tool list until they reconnect or ask for it again — this isn't instant for them.",
    changedBanner: (name, to) => `"${name}" is now ${to}.`,
    changeFailed: (message) => `Couldn't apply the change: ${message}`,
    sourceHeader: "Source",
    sourceNative: "Built-in",
    sourceExternal: (label) => `External — ${label}`,
  },
  connections: {
    title: "External MCP servers",
    description: "Remote MCP servers connected to this one, whose tools are offered to any assistant connected here, alongside the built-in ones.",
    newLink: "Connect a new server",
    empty: "No external servers connected yet.",
    labelHeader: "Label",
    urlHeader: "URL",
    statusHeader: "Status",
    catalogHeader: "Tools",
    enabledLabel: "enabled",
    disabledLabel: "disabled",
    toolCount: (n) => `${n} tool${n === 1 ? "" : "s"}`,
    lastFetched: (when) => `last checked ${when}`,
    neverFetched: "never successfully reached",
    collisionNotice: (toolName, connectionLabel) =>
      `"${toolName}" from "${connectionLabel}" wasn't registered — a tool with that name already exists.`,
    errorCodeLabel: (code) => {
      switch (code) {
        case "external_unreachable":
          return "Could not reach the server";
        case "external_timeout":
          return "The server didn't respond in time";
        case "external_unauthorized":
          return "The server rejected the stored token";
        case "external_invalid_response":
          return "The server returned an invalid response";
        default:
          return code;
      }
    },
    editAction: "Edit",
    enableAction: "Enable",
    disableAction: "Disable",
    refreshAction: "Refresh now",
    removeAction: "Remove",
    changedBanner: (label, status) => `"${label}" is now ${status}.`,
    newTitle: "Connect an external MCP server",
    editTitle: "Edit connection",
    labelFieldLabel: "Label",
    labelPlaceholder: "e.g. Order Management",
    urlFieldLabel: "Server URL",
    urlPlaceholder: "https://example.com/mcp",
    tokenFieldLabel: "Bearer token",
    tokenPlaceholder: "Paste the token",
    tokenWriteOnlyNotice: "The current token is never shown again once saved — leave this blank to keep it unchanged, or enter a new one to replace it.",
    submitCreate: "Connect",
    submitEdit: "Save changes",
    invalidUrl: "Enter a valid http(s):// URL.",
    missingFields: "Label, URL, and token are all required.",
    confirmTitle: "Confirm change",
    confirmEnable: (label) => `You're about to enable "${label}" — its tools will become available again.`,
    confirmDisable: (label) => `You're about to disable "${label}" — its tools will stop being offered, but its saved configuration is kept.`,
    confirmRemove: (label) => `You're about to permanently remove "${label}", including its saved token. This can't be undone.`,
    confirmButton: "Confirm",
    cancelButton: "Cancel",
    warningNotice: "AI assistant sessions that were already connected before this change may keep using the previous tool list until they reconnect or ask for it again — this isn't instant for them.",
    removeWarning: "You'll need to re-enter the URL and token to reconnect this server later.",
    changeFailed: (message) => `Couldn't apply the change: ${message}`,
  },
  schedules: {
    title: "Scheduled tasks",
    description: "Tasks that run automatically on a schedule, each with its own prompt and assigned model.",
    newLink: "New task",
    empty: "No scheduled tasks yet.",
    nameHeader: "Name",
    cronHeader: "Schedule",
    modelHeader: "Model",
    statusHeader: "Status",
    lastRunHeader: "Last run",
    enabledLabel: "enabled",
    disabledLabel: "disabled",
    neverRun: "never run",
    editAction: "Edit",
    enableAction: "Enable",
    disableAction: "Disable",
    removeAction: "Delete",
    changedBanner: (name, to) => `"${name}" is now ${to}.`,
    removedLabel: "deleted",
    confirmTitle: "Confirm",
    confirmRemove: (name) => `Delete the scheduled task "${name}"? This can't be undone.`,
    removeWarning: "The task definition is deleted; its past execution history is kept.",
    confirmButton: "Confirm",
    cancelButton: "Cancel",
    changeFailed: (message) => `Couldn't apply the change: ${message}`,
    newTitle: "New scheduled task",
    editTitle: "Edit scheduled task",
    nameFieldLabel: "Name",
    namePlaceholder: "e.g. Weekly report",
    cronFieldLabel: "Cron expression",
    cronPlaceholder: "0 8 * * 1",
    timezoneFieldLabel: "Timezone (optional)",
    timezonePlaceholder: "Europe/Rome",
    modelFieldLabel: "Model",
    promptFieldLabel: "Prompt",
    promptPlaceholder: "Describe what this task should do...",
    enabledFieldLabel: "Enabled",
    submitCreate: "Create task",
    submitEdit: "Save changes",
    validationError: (message) => message,
    historyTitle: "Execution history",
    historyEmpty: "This task hasn't run yet.",
    historyLink: "History",
    backLink: "Back to scheduled tasks",
    triggerScheduled: "scheduled",
    triggerManual: "manual",
    statusSuccess: "success",
    statusFailure: "failure",
    startedAtHeader: "Started",
    summaryHeader: "Summary",
    runNowAction: "Run now",
    runNowAlreadyRunning: "This task is already running.",
  },
  oauth: {
    login: {
      title: "Sign in",
      description: "Sign in as the owner to approve connections and manage connected apps.",
      username: "Username",
      password: "Password",
      submit: "Sign in",
      errorInvalidCredentials: "Incorrect username or password.",
      errorLockedOut: "Too many failed attempts — try again in a few minutes.",
      errorGeneric: "Sign-in failed.",
    },
    authorize: {
      title: "Authorize access",
      requesting: (clientName) => `${clientName} is requesting access to your MCP storage tools.`,
      approve: "Approve",
      deny: "Deny",
      cantContinue: "Can't continue",
      errorMissingParams: "Missing required parameters (client_id, redirect_uri, code_challenge).",
      errorUnsupportedChallenge: "Only the S256 code_challenge_method is supported.",
      errorUnknownClient: "Unknown client or redirect_uri.",
    },
  },
};
